...
def parse_arguments():
    ...
    parser.add_argument("-l", "--loci", help="Optional: Path to file containing genes/loci of interest (single column with header 'locus_tag' or 'gene')")
...
def filter_guides_parallel(guides_df, coordinates_df, chromosome_sizes, proximal, distal, n_processes=None):
    """Parallel implementation of guide filtering"""
    # First filter for intergenic guides with no gene targets
    guides_df = guides_df[
        (guides_df['intergenic'] == 1) & 
        (guides_df['genes'] == 0)
    ]
    
    print(f"\nFiltered to {len(guides_df)} intergenic guides with no gene targets")
...
def write_outputs(filtered_guides, output_path, coordinates_df, missing_loci=None, loci_df=None):
    ...
    report_guides_per_gene(filtered_guides, output_path, coordinates_df, loci_df)
...
def main():
    try:
        ...
        # Write outputs, passing coordinates_df and loci information
